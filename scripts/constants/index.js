const PACKAGE_DIRECTORY_NAME_PREFIX = 'tkeel-console-';
const PLUGIN_PACKAGE_DIRECTORY_NAME_PREFIX = `${PACKAGE_DIRECTORY_NAME_PREFIX}plugin-`;
const PACKAGE_NAME_PREFIX = '@tkeel/console-';
const PLUGIN_PACKAGE_NAME_PREFIX = `${PACKAGE_NAME_PREFIX}plugin-`;

const PORTAL_PACKAGE_SIMPLE_NAME = 'portal';
const PORTAL_PACKAGE_DIRECTORY_NAME = `${PACKAGE_DIRECTORY_NAME_PREFIX}${PORTAL_PACKAGE_SIMPLE_NAME}`;
const PORTAL_PACKAGE_NAME = `${PACKAGE_NAME_PREFIX}${PORTAL_PACKAGE_SIMPLE_NAME}`;

const SHARED_PACKAGE_SIMPLE_NAMES = [
  'business-components',
  'components',
  'hooks',
  'icons',
  'themes',
  'types',
  'utils',
];

const COMMON_PACKAGE_SIMPLE_NAMES = [
  PORTAL_PACKAGE_SIMPLE_NAME,
  ...SHARED_PACKAGE_SIMPLE_NAMES,
];

const EXCLUDE_PACKAGE_DIRECTORY_NAMES = ['types-tkeel-console'];

const PORTAL_PACKAGE_INFOS = [
  {
    platform: 'admin',
    devServerPort: '3000',
  },
  {
    platform: 'tenant',
    devServerPort: '3001',
  },
];

module.exports = {
  PACKAGE_DIRECTORY_NAME_PREFIX,
  PLUGIN_PACKAGE_DIRECTORY_NAME_PREFIX,
  PACKAGE_NAME_PREFIX,
  PLUGIN_PACKAGE_NAME_PREFIX,
  PORTAL_PACKAGE_SIMPLE_NAME,
  PORTAL_PACKAGE_DIRECTORY_NAME,
  PORTAL_PACKAGE_NAME,
  SHARED_PACKAGE_SIMPLE_NAMES,
  COMMON_PACKAGE_SIMPLE_NAMES,
  EXCLUDE_PACKAGE_DIRECTORY_NAMES,
  PORTAL_PACKAGE_INFOS,
};
